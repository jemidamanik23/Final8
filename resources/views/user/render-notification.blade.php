@if(isset($notifications) && $notifications && count($notifications) > 0)
            ...
            <div class="media-body">
                ...
                <div class="media-body-content">
                    ...
                    @if(!$notification->read_at)
                        <a class="text-success" href="{{ route('user.read_notification',$notification->id) }}">READ_NOTIFICATION</a>
                    @endif
                    <a class="text-danger" href="{{ route('user.delete_notification',$notification->id) }}">REMOVE</a>
                </div>
            </div>
        </div>
        <hr>
    ...
    <div class="btn-group d-flex justify-content-center">
        <a class="btn btn-outline-success mt-3" href="{{ route('user.read_notification', ["all", 'type'=>$notifications[0]->type]) }}">
            <i class="fa fa-eye fw mr-2"></i>READ_ALL_NOTIFICATION $unreadCount 
        </a>
        <button class="btn btn-outline-danger mt-3 swal-dialog-target"
                ...
                data-method="delete">
            <i class="fa fa-trash fw mr-2"></i>DELETE_READ<span class="badge badge-danger">{{ $readCount }}</span>
        </button>
    </div>
@else
