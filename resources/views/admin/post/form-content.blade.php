<div class="form-group">
    <label for="cover_img" class="form-control-label">COVER_IMAGE</label>
...
<div class="form-group">
    <label for="title" class="form-control-label">ARTICLE_TITLE*</label>
...
<div class="form-group">
    <label for="description" class="form-control-label">ARTICLE_DESCRIPTION</label>

    <textarea id="post-description-textarea" style="resize: vertical;" rows="3" spellcheck="false"
              id="description" class="form-control autosize-target{{ $errors->has('description') ? ' is-invalid' : '' }}" placeholder="descripsi salah"
...
<div class="form-group">
    <label for="slug" class="form-control-label">ARTICLE_SLUG*</label>
...
<div class="form-group">
    <label for="categories" class="form-control-label">ARTICLE_CATEGORY*</label>
...
<div class="form-group">
    <label for="tags[]" class="form-control-label">ARTICLE_TAGS</label>
...
<div class="form-group">
    <label for="post-content-textarea" class="form-control-label">ARTICLE_CONTENT*</label>
    <textarea data-save-id="{{ isset($post)?'post.edit.'.$post->id.'.by@' . request()->ip():'post.create' }}" id="simplemde-textarea"
              class="form-control{{ $errors->has('content') ? ' is-invalid ' : ' ' }}"
              name="content"
              spellcheck="false"
              rows="36"
              placeholder="USE_MARKDOWN_WRITE"
...
<div class="mb-3" style="font-size: 80%">
    <a class="text-secondary font-italic" data-toggle="collapse" href="#post-extra-info" role="button" aria-expanded="false">
        <span title="COMMENT_MESSAGE" data-toggle="tooltip">OTHER_MESSAGE	&raquo;</span>
    </a>
</div>
<div class="collapse" id="post-extra-info">
    <div class="form-group">
        <label for="comment_info" class="form-control-label">OTHER_MESSAGE</label>
        <select style="margin-top: 5px" id="comment_info" name="comment_info" class="form-control">
            ...
            <option value="force_disable" {{ $comment_info=='force_disable'?' selected' : '' }}>FORCE_DISABLE_COMMENT</option>
            <option value="force_enable" {{ $comment_info=='force_enable'?' selected' : '' }}>FORCE_ENABLE_COMMENT</option>
        </select>
    </div>
    <div class="form-group">
        <label for="comment_type" class="form-control-label">COMMENT_TYPE</label>
        <select id="comment_type" name="comment_type" class="form-control">
            ...
            <option value="raw" {{ $comment_type=='raw'?' selected' : '' }}>COMMENT_RAW</option>
            <option value="disqus" {{ $comment_type=='disqus'?' selected' : '' }}>Disqus</option>
        </select>
    </div>

    <div class="form-group">
        <label for="allow_resource_comment" class="form-control-label">ALLOW_RESOURCE_COMMENT</label>
        <select id="allow_resource_comment" name="allow_resource_comment" class="form-control">
            ...
            <option value="false" {{ $allow_resource_comment=='false'?' selected' : '' }}>DISALLOW_COMMENT</option>
            <option value="true" {{ $allow_resource_comment=='true'?' selected' : '' }}>ALLOW_COMMENT</option>
        </select>
    </div>

    <div class="form-group">
        <label for="enable_toc" class="form-control-label">IS_SHOW_TOC</label>
        <select id="enable_toc" name="enable_toc" class="form-control">
            <?php $enable_toc = isset($post) ? $post->getConfig('enable_toc', 'true') : 'true'?>
            <option value="false" {{ $enable_toc=='false'?' selected' : '' }}>CLOSE</option>
            <option value="true" {{ $enable_toc=='true'?' selected' : '' }}>SHOW</option>
        </select>
    </div>
</div>
