...
@section('action')
    @if($unverified_count>0)
        <button class="btn btn-sm btn-outline-danger swal-dialog-target"
                data-dialog-msg="{REMOVE} {{ $unverified_count }} {UNVERIFIED_COMMENT}"
                data-url="{{ route('comment.delete-un-verified', ['ids'=>$unverified_ids]) }}"
                data-method="delete">DELETE_UNVERIFIED
        </button>
    @endif
@endsection
@if($comments->isEmpty())
    ...
    <table class="table table-striped" id="comments-table">
        ...
        @foreach($comments as $comment)
            ...
            <tr id="{{ $comment->id }}">
                ...
                <td>
                    <span class="p-2 p badge {{ $comment->trashed() ? 'badge-danger':($comment->isVerified() ? 'badge-success' : 'badge-secondary') }}">{{ $comment->trashed() ? __('IS_REMOVE'):($comment->isVerified() ? __('IS_VERIFIED') : __('NOT_VERIFIED')) }}</span>
                </td>
                ...
                <td>
                    @if($comment->trashed())
                        <button type="submit"
                                ...
                                data-dialog-msg="DELETE_THIS_COMMENT_FOREVER？"
                    ...
                    @else
                        <button type="submit"
                                class="btn btn-danger swal-dialog-target"
                                data-dialog-msg="SURE_DELETE_THIS_COMMENT？"
                                data-toggle="tooltip"
