...
class PostController extends Controller
{
    ...
    public function create()
    {
        if ($this->categoryRepository->count() <= 0) {
            return redirect()->route('admin.categories')->withErrors(PLEASE_CREATE_CATEGORY);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($post) {
            ...
            return redirect('admin/posts')->with('success', ARTICLE . "<a href='$link'>$post->title</a>" . CREATE_SUCCESS);
        } else {
            return redirect('admin/posts')->withErrors(ARTICLE . $request['name'] . CREATE_FAIL);
        }
    }
    ...
    public function publish($id)
    {
        ...
        if ($post->trashed()) {
            return back()->withErrors($post->title . __('PUBLISH_FAIL_REMOVE'));
        }
        $this->clearAllCache();
        if ($post->status == 0) {
            $post->status = 1;
            ...
            if ($post->save()) {
                ...
                return back()->with('success', "<a href='$link'>$post->title</a> " . __('PUBLISH_SUCCESS'));
            }
        } else if ($post->status == 1) {
            $post->status = 0;
            if ($post->save()) {
                $link = $this->getPostLink($post);
                return back()->with('success', "<a href='$link'>$post->title</a> " . __('REVOKE_PUBLISH_SUCCESS'));
            }
        }
        return back()->withErrors($post->title . __('OPERATING_FAIL'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($this->postRepository->update($request, $post)) {
            $link = $this->getPostLink($post);
            return redirect('admin/posts')->with('success', "<a href='$link'>$post->title</a> " . __('EDIT_SUCCESS'));
        } else
            return redirect('admin/posts')->withErrors(__('ARTICLE') . $request['name'] . __('EDIT_FAIL'));
    }
    ...
    public function restore($id)
    {
        ...
        if ($post->trashed()) {
            ...
            $this->clearAllCache();
            $link = $this->getPostLink($post);
            return redirect()->route('admin.posts')->with('success', "<a href='$link'>$post->title</a>" . __('RECOVERY_SUCCESS'));
        }
        return redirect()->route('admin.posts')->withErrors(__('RECOVERY_FAIL'));
    }
    ...
    public function destroy($id)
    {
        ...
        if ($result) {
            $this->clearAllCache();
            return redirect($redirect)->with('success', __('REMOVE'). $post->title.__('web.SUCCESS'));
        } else
            return redirect($redirect)->withErrors(__('REMOVE_FAIL'));
    }
