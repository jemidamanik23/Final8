...
class CommentController extends Controller
{
    ...
    public function update(Request $request, $comment_id)
    {
        ...
        if ($this->commentRepository->update($request->get('content'), $comment)) {
            ...
            if ($redirect)
                return redirect($redirect)->with('success', __('EDIT_SUCCESS'));
            return back()->with('success', __('EDIT_SUCCESS'));
        }
        return back()->withErrors(__('EDIT_FAIL'));
    }
    ...
    public function destroy($comment_id)
    {
        ...
        if ($this->commentRepository->delete($comment, $force)) {
            ...
            return back()->with('success', __('REMOVE_SUCCESS'));
        }
        if (request()->expectsJson()) {
            return response()->json(['status' => 500, 'msg' => __('REMOVE_FAIL')]);
        }
        return back()->withErrors(__('REMOVE_FAIL'));
    }
